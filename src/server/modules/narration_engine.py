...
from src.shared.types import NarrationPack, Subtitle, TextCard
...
class NarrationEngine:
    ...
    @staticmethod
    def generate_narration(project_id: str, timeline_id: str, tts_voice: str) -> NarrationPack:
        """
        ...
        try:
            ...
            narration_text = NarrationEngine._generate_narration_text(project_id, session)

            # Call TTS API
            tts_audio_path, audio_duration = NarrationEngine._call_tts_api(
                narration_text, tts_voice, project_id
            )

            # Generate subtitles
            subtitles = NarrationEngine._generate_subtitles(narration_text, audio_duration)

            # Generate text cards
            text_cards = NarrationEngine._generate_text_cards(project_id, session)

            # Create NarrationPack
            narration_id = str(uuid.uuid4())
            version_id = ArtifactStore.create_version(
                "narration", project_id, upstream_versions={"timeline": timeline_id}
            )
    ...
    def _generate_narration_text(project_id: str, session) -> str:
        ...
        segments = (
            session.query(StorySegmentRecord)
            .filter_by(project_id=project_id)
            .order_by(StorySegmentRecord.start_index)
            .all()
        )
    ...
    @staticmethod
    def _call_tts_api(text: str, voice: str, project_id: str) -> Tuple[str, float]:
        """
    ...
    @staticmethod
    def _generate_subtitles(text: str, audio_duration: float) -> List[Subtitle]:
        """Generate subtitles from narration text."""
        words = text.split()
        words_per_chunk = max(
            1, int(len(words) * NarrationEngine.SUBTITLE_CHUNK_DURATION / audio_duration)
        )
    ...
    @staticmethod
    def _persist_narration(project_id: str, narration_pack: NarrationPack, session) -> None:
